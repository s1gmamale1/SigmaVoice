...
### ADR-007 — Opt-in remote STT (self-hosted/LAN) + cloud LLM cleanup (OpenRouter)
**Decision (approved 2026-06-14, operator).** Authorize, **off by default + explicit opt-in**: **(a)** remote speech-to-text to a user-configured **OpenAI-compatible** endpoint, **scoped to self-hosted / LAN servers** (e.g. the operator's RTX 3060 box running `faster-whisper-server`/`whisper.cpp` `whisper-server`); and **(c)** a cloud **LLM transcript-cleanup/transform** pass via **OpenRouter** (`/api/v1/chat/completions`). Both ship **off by default**, behind explicit Settings toggles, **account-free** (bring-your-own endpoint/key), with **auto-fallback to on-device Whisper** on STT failure and **raw-passthrough** on cleanup failure. **NOT** authorized here: **(b)** third-party cloud STT vendors (Groq/OpenAI audio upload) — deferred to a future ADR. This is the **new ADR that ADR-001 requires for cloud-transcription opt-in** (scoped to self-hosted/LAN) and the one **ADR-002 deferred for a cloud LLM cleanup** — it supersedes ADR-002's deferral for case (c). Implements roadmap **FE-3 (cloud variant)** + a new **FE-15 (remote STT)** / **FE-16 (OpenRouter cleanup)**. Full design: [`_plans/2026-06-14-remote-stt-and-openrouter-cleanup-design.md`](./_plans/2026-06-14-remote-stt-and-openrouter-cleanup-design.md).
**Context.** Operator runs Whisper on an RTX 3060 and wants SigmaVoice to dictate against it, plus a BridgeVoice-style LLM cleanup pass. Research (2026-06-14) confirmed: BridgeVoice/SigmaLink **never actually wired OpenRouter** (inert `ModelTransport='openrouter'` enum, zero git history); `voice-core` already ships an OpenAI-compatible STT engine (`cloud-stt-engine.ts`, URL hardcoded to `api.openai.com`) and the capture controller already routes to it via `voice.transcriptionMode`. Self-hosted LAN STT keeps audio on hardware the **operator controls** — a materially weaker privacy concession than third-party cloud, consistent with ADR-001's account-free spirit.
**Consequences.** (+) Unlocks GPU-class STT (large-v3) + LLM formatting with **no backend/account**; LAN STT keeps audio on the operator's own machine. (+) Reuses existing seams — a **configurable base URL** on the existing `openai-whisper-api` mode, and the post-`normalizeTranscript` cleanup seam (`global-capture.ts:596`); no new heavy deps (global `fetch` + Electron `safeStorage`). (−) Audio/text **leave the device when enabled** → UI must be loud, explicit, default-off; the OpenRouter API key is stored **encrypted** (safeStorage/Keychain), not plaintext KV. (−) HTTP paths need their **own fetch timeout + AbortController** (ADR-002's ENG-3 spawn-timeout prereq is CLI-specific; satisfied here by fetch abort). (−) Engine changes are the **two-repo dance** (ADR-003): authored in SigmaLink `voice-core` → pin bump. (−) Third-party cloud STT (b) stays gated behind a future ADR.
